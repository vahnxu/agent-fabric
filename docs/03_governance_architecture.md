...
## 4. 移动总机模式与 Fail-Closed 原子换壳自愈（Switchboard & Auto-Rotation）

```mermaid
flowchart TD
    A["监测到总机转录超过阈值（默认 50MB）"] --> B["后台冷起全新干净的总机 Session (临时名)"]
...
2. **阈值监控**：守护进程监控总机转录文件大小，默认 50MB（`--max-mb` 可调）；
3. **顺序即防线**：影子会话**必须先在真实消息通道上应答**，之后才轮到旧会话退役。这个顺序是整个机制的全部——任何时刻在线的服务方数量都不为零；
4. **失败即回滚，但退役失败不回滚**：启动失败、握手不应答、改名失败，一律销毁影子、旧会话继续服务（`rolled_back`）。而**旧会话退不掉**时新会话已经在服务了，此时回滚才是真正的断线——机制把它报成 `retire_failed`（留一个待收的僵尸进程），而不是失败；
5. **死窗必须上报**：重连后主动给出「从几点到几点没有人在服务」的区间。不说明自己漏了什么的重连，比可见的宕机更糟——它看起来像什么都没发生。

参考实现 `core/switchboard_supervisor.py` 把上述五点做成了可注入的状态机：怎么起会话、怎么探活、怎么改名、怎么退役全部由调用方以 callable 传入，模块本身不认识任何运行时。
